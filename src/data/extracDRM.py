...
import requests
url = 'https://confluence.amlogic.com/display/SW/DRM+FAQ'
response = requests.get(url, headers=headers)

from bs4 import BeautifulSoup
soup = BeautifulSoup(response.text, 'html.parser')

questions = soup.find_all('h2', {'id': lambda x: x and 'DRMFAQ' in x})
cleanQuesions = []
cleanAnswers = []
...
print(df)
df.to_csv('src/data/drm.csv', sep='|',quotechar='\'',index=False)
